...
class SegmentationModelS(SegmentationModel):
    def forward(self, x):
        # S 모델의 forward 방식 - T 모델과 다른 동작 정의
        self.check_input_shape(x)
        features = self.encoder(x)

        # 여기에 S 모델만을 위한 추가적인 전처리나 과정 정의
        # 예를 들어 CAM 모듈을 추가하지 않고 일반적인 디코딩 진행
        decoder_output = self.decoder(*features)
        final_result = self.segmentation_head(decoder_output)

        return final_result, decoder_output
...
class Unet(SegmentationModel):
    ...
    def __init__(
        ...
        # def forward(self, x):
        #     """Override forward to add 3 ADCARB blocks in series at bottleneck"""
        #
        #     # Encoder forward pass
        #     features = self.encoder(x)
        #
        #     # # Apply 3 ADCARB blocks in series at the bottleneck
        #     # bottleneck = features[-1]  # The bottleneck feature from the encoder
        #     # bottleneck = self.adcarb1(bottleneck)
        #     # bottleneck = self.adcarb2(bottleneck)
        #     # bottleneck = self.adcarb3(bottleneck)
        #
        #     # # Replace the bottleneck feature with the ADCARB output
        #     # features[-1] = bottleneck
        #
        #     # Decoder forward pass using modified bottleneck
        #     decoder_output = self.decoder(*features)
        #
        #     # Only return the decoder output (skip segmentation head)
        #     return decoder_output

# Unet 모델을 생성 (unet_model.py에서 정의된 클래스)
# model = Unet(
#     encoder_name="resnet50",  # 사용할 인코더 이름
#     encoder_depth=5,          # 인코더 깊이
#     encoder_weights="imagenet",  # ImageNet에서 사전 학습된 가중치 사용
#     decoder_use_batchnorm=True,
#     decoder_channels=[256, 128, 64, 32, 16],  # 디코더 채널 크기
#     in_channels=7,  # 입력 채널 수를 7로 설정
#     classes=3,
#     activation=None
# )
...
#
# # 입력 텐서를 GPU로 이동
# input_tensor = torch.randn(1, 7, 256, 256).to(device)
#
# # FLOPs와 파라미터 계산
